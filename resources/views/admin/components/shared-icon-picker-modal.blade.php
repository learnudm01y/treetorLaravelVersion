{{--
    Shared Icon Picker Modal - Include this once in the layout
    Usage: Call window.openIconPicker(callback) from anywhere
--}}

<div x-data="sharedIconPicker()"
     x-show="isOpen"
     x-cloak
     @open-icon-picker.window="openModal($event.detail)"
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
        <div x-show="isOpen"
            ...
            <div class="flex-1 overflow-y-auto p-6" x-ref="iconsContainer" @scroll="handleScroll()">
                {{-- No Results --}}
                <div x-show="displayedIcons.length === 0" class="text-center py-12">
                ...
                {{-- Icons Grid --}}
                <div x-show="displayedIcons.length > 0" class="icon-picker-grid">
                    <template x-for="icon in displayedIcons" :key="icon.class">
                        <button type="button"
                                @click="selectIcon(icon.class)"
                                :class="tempSelection === icon.class ? 'icon-picker-selected' : 'icon-picker-normal'"
                                :title="icon.name"
...
.dark .icon-picker-normal {
    background-color: #1f2937;
    color: #d1d5db;
}
.dark .icon-picker-normal:hover {
...
document.addEventListener('alpine:init', () => {
    Alpine.data('sharedIconPicker', () => ({
        ...
        openModal(detail) {
            ...
            this.tempSelection = detail.currentValue || '';
            this.searchQuery = '';
        ...
        handleScroll() {
            ...
            if (container) {
                ...
                if (scrollBottom < 100 && this.hasMore) this.loadMoreIcons();
            }
        },
        ...
        confirmSelection() {
            if (this.tempSelection && this.callback) {
                this.callback(this.tempSelection);
            }
            this.closeModal();
        }
    }));
...
// Global function to open icon picker
window.openIconPicker = function(currentValue, callback) {
    window.dispatchEvent(new CustomEvent('open-icon-picker', {
        detail: { currentValue, callback }
    }));
};
</script>
